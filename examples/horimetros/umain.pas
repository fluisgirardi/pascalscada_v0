...
uses
  ...
  ExtCtrls, StdCtrls, ComCtrls, Buttons, HMIAnimation, PLCTagNumber, HMIEdit,
  HMILabel, HMICheckBox, HMIText;
...
begin
  if userlevel<=0 then begin
    dlglogin:=TfrmLogin.Create(self);
    try
      if dlglogin.ShowModal=mrOK then begin
        if dmdb.Login(dlglogin.edtLogin.Text, dlglogin.edtPassword.Text) then begin
