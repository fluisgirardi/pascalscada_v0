...
function driverinstall2(const path,name:pchar;const dwStartType:cardinal):integer;
...
function driverinstall(const path,name:pchar):integer;
begin
 Result := driverinstall2(path,name,SERVICE_DEMAND_START);
end;
...
begin
  ...
  hService := CreateService(hSCMan, name,name,
              SERVICE_ALL_ACCESS, SERVICE_KERNEL_DRIVER, dwStartType,
...
var HRs:HRSRC;
    ...
    cbr:cardinal;
begin
 HRs := FindResource(0,from,RT_RCDATA);
 HDrvRes := LoadResource(0,HRs);
 Size := SizeofResource(0,HRs);
 drv := LockResource(HDrvRes);
 HFile := CreateFile(tofile,GENERIC_WRITE,
   FILE_SHARE_WRITE,nil,CREATE_ALWAYS,FILE_ATTRIBUTE_ARCHIVE, 0);
 writeFile(HFile,drv,size,cbr,nil);
 if cbr = size then result := 0 else result := getlasterror;
 CloseHandle(hFile);
end;
